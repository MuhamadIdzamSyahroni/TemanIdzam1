...
class UpdateData : AppCompatActivity() {
        ...
        update.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                ...
                if (isEmpty(cekNama!!) || isEmpty(cekAlamat!!) || isEmpty(cekNoHP!!)) {
                    ...
                    val setteman_idzam = teman_idzam()
                    setteman_idzam.nama= new_nama.getText().toString()
                    setteman_idzam.alamat= new_alamat.getText().toString()
                    setteman_idzam.no_hp = new_nohp.getText().toString()
                    updateTeman(setteman_idzam)
                }
            }
        })
    }
    ...
    private fun updateTeman(teman: teman_idzam) {
        var userID = auth!!.uid
